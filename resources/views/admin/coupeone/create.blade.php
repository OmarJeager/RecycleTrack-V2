<div class="container" style="max-width: 600px; margin-top: 50px;">
    ...
    <form action="{{ route('coupeone.store') }}" method="POST" style="background-color: #f9f9f9; padding: 20px; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
        @csrf
            ...
            @foreach ($machines as $machine)
                <tr style="background-color: #f9f9f9; transition: background-color 0.3s;">
                    ...
                    <td style="padding: 10px; border-bottom: 1px solid #ccc;">
                        <a href="{{ route('coupeone.edit', [$machine->id]) }}" style="background-color: #ffc107; color: white; padding: 5px 10px; border-radius: 4px; border: none; cursor: pointer; text-decoration: none; transition: background-color 0.3s;">Edit</a>
                        <form action="{{ route('coupeone.delete', [$machine->id]) }}" method="POST" style="display: inline-block;" onsubmit="return confirmDelete('{{ $machine->machines }}')">
                            @csrf
                            @method('DELETE')
